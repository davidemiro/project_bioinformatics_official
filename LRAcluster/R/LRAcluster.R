...
LRAcluster<-function(data, dimension)
{
  types = list('gaussian','poisson', 'poisson')
  ...
  return (list("coordinate"=t(coordinate),"potential"=ratio)) #aggiunto T()
}
...
unionMatrix2 <- function(x,y){
    matrixList <- list(x,y)
    return (matrixList)
}

unionMatrix3 <- function(x,y,z){
    return (list(x,y,z))
}
